...
export const createOrderSchemas = z.object({
  packageId: z.string(), // userId: số nguyên dương
  userId: z.string().optional(), // userId: số nguyên dương
  quantity: z.number().int().nonnegative(), // points: số nguyên >= 0
  coupon_code: z.string().optional(), // points: số nguyên >= 0
});


// IndexQuerySchema
const StatusEnum = z.enum(['pending', 'completed', 'failed', 'canceled', 'new', 'expired']);
...
export const depositQuerySchema = z.object({
  ...
  _type_package: z.string().optional(),
...
export type CeateOrderSchemasInput = z.infer<typeof createOrderSchemas>;
