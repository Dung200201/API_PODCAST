...
export const getAllBlog20Request = async (
  ...
  try {
    ...
    // fix isAdmin logic: chỉ dựa trên role của user hiện tại
    const isAdmin =
    ...
    const [requests, totalItems] = await Promise.all([
      ...
      fastify.prisma.blog20Request.count({ where }),
    ]);
    ...
    if (!requests || requests.length === 0) {
      return reply.status(200).send({
        message: "No blog20 requests found.",
        success: true,
        pagination,
        blog20s: [],
      });
    ...
    // 1️⃣ Lấy danh sách blogGroupId của các request "register"
    const registerGroupIds = [
      ...new Set(
        requests
          .filter((r: any) => r.typeRequest === "register" && r.blogGroupId)
          .map((r: any) => r.blogGroupId)
      ),
    ];
    ...
    if (registerGroupIds.length > 0) {
      // 2️⃣ Query tất cả link thuộc request type="post" có blogGroupId trong danh sách trên
      const postRequests = await fastify.prisma.blog20Request.findMany({
        where: {
          blogGroupId: { in: registerGroupIds },
          typeRequest: "post",
          deletedAt: null,
        },
        select: {
          blogGroupId: true,
          blog20_link: { select: { id: true } },
        },
      });

      // 3️⃣ Gom tổng số link theo blogGroupId
      linkCountByGroup = postRequests.reduce((acc, r: any) => {
        const count = r.blog20_link?.length || 0;
        acc[r.blogGroupId] = (acc[r.blogGroupId] || 0) + count;
        return acc;
      }, {} as Record<string, number>);
    }

    // LẤY TOTAL ACCOUNT LIVE THEO BLOGGROUPID
    const allGroupIds = [
      ...new Set(requests.filter((r: any) => r.blogGroupId).map((r: any) => r.blogGroupId))
    ];

    let accountCountMap: Record<string, number> = {};

    if (allGroupIds.length > 0) {
      const accountsByGroup = await fastify.prisma.blog20Account.groupBy({
        by: ["blogGroupId"],
        where: {
          blogGroupId: { in: allGroupIds },
          status: "live",
          deletedAt: null,
        },
        _count: { blogGroupId: true },
      });

      accountCountMap = accountsByGroup.reduce((acc, item) => {
        acc[item.blogGroupId] = item._count.blogGroupId;
        return acc;
      }, {} as Record<string, number>);
    }
    ...
    const formatted = requests.map((r: any) => {
      let total_links = Array.isArray(r.blog20_link)
        ? r.blog20_link.length
        : 0;

      // nếu là "register" thì lấy tổng link của các request post cùng blogGroupId
      if (r.typeRequest === "register" && r.blogGroupId) {
        total_links = linkCountByGroup[r.blogGroupId] || 0;
      }

      const { blog20_link, ...rest } = r;
      return {
        ...rest,
        total_links,
        totals_account: accountCountMap[r.blogGroupId] || 0
      };
    });
