...
export const getAllPodcastGroup = async (
  ...
  try {
    ...
    // **Truy vấn dữ liệu từ bảng PodcastGroup**
    const [podcastGroups, totalItems] = await Promise.all([
      fastify.prisma.podcastGroup.findMany({
        where,
        select: {
          id: true,
          name: true,
      ...
      fastify.prisma.podcastGroup.count({ where }),
    ]);
