...
const querySchema = z.object({
  ...
  _blogRequestId: z.string().optional(),
  _s: z.string().optional(), // search keyword
});

export const getAllBlog20Link = async (
  ...
  try {
    ...
    // ✅ Base filter
    const where: any = {};
    if (_blogRequestId) where.blogRequestId = _blogRequestId;
    ...
    const [blogLinks, totalItems] = await Promise.all([
      fastify.prisma.blog20Link.findMany({
        where,
        select: {
          id: true,
          blogRequestId: true,
      ...
      fastify.prisma.blog20Link.count({ where }),
    ]);

    // Tính hiệu suất khi get theo _blogRequestId 
    let performanceRate: number | null = null;
    let totalLinks: number | undefined;
    if (_blogRequestId) {
      const requestData = await fastify.prisma.blog20Request.findUnique({
        where: { id: _blogRequestId },
        select: { target: true },
      });
    ...
    // ✅ Nếu có _blogRequestId thì đổi trường total và thêm performanceRate
    if (_blogRequestId) {
